...
Likely exploitable (VER) & \code{LEV}: exploit maturity / EPSS threshold \\
...
\item $m,\ m^{*}$ --- does the system serve one agency ($0$) or many ($1$)?
  $m^{*}$ adds the ``shared-infrastructure'' escalation.
...
Let $m \in \{0,1\}$ be the asset's multi-agency flag. Some asset classes are
\emph{scope amplifiers} (shared infrastructure whose compromise crosses tenant
boundaries); for those, an offering that itself serves more than one agency
($\textsc{cso}_{>1}$) is treated as multi-agency regardless of the per-asset flag:
\begin{equation}
m^{*} \;=\; m \ \lor\ \big(\text{amplifier} \ \land\ \textsc{cso}_{>1}\big).
\label{eq:scope}
\end{equation}
...
The day-valued tables are given in Appendix~\ref{app:matrix}. PAIN-1 carries no
FedRAMP deadline.
...
archetypes}, each carrying a fixed CR/IR/AR and an amplifier flag, and to tag each
asset with one archetype.
...
agencies at the highest assurance (Class~D). Because the shared components carry more
than one agency's data, the scope flag is set on them, and the amplifier types
(\code{identity-secrets}, \code{data-backbone}, \code{config-actuation},
\code{platform-foundation}) escalate accordingly. Table~\ref{tab:scand} shows the
result: the same families of flaws now remediate in \emph{hours}.
...
agencies, so amplifier types escalate to multi-agency scope.}
\label{fig:archd}
\end{figure}
...
\label{app:catalog}
Illustrative only (see Section~\ref{sec:archetypes}). ``Amp'' marks scope amplifiers.
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l l c c c c >{\raggedright\arraybackslash}p{0.30\textwidth}}
\toprule
\textbf{Archetype} & \textbf{Lens} & \textbf{CR} & \textbf{IR} & \textbf{AR} & \textbf{Amp} & \textbf{Typical members} \\
\midrule
\code{cicd-pipeline}      & control & H & H & H & \checkmark & build/deploy runners, artifact signing, registries \\
\code{orchestrator}       & control & H & H & H & \checkmark & control plane, etcd, scheduler, coordination \\
\code{config-actuation}   & control & H & H & H & \checkmark & IaC/GitOps, schema registry, admin/migration \\
\code{identity-secrets}   & control & H & H & H & \checkmark & IdP/SSO, KMS, secrets managers, session stores \\
...
\code{platform-foundation}& control & L & H & H & \checkmark & DNS, NTP, service discovery, L4 internal LBs (metadata only) \\
\code{data-sensitive}     & data    & H & H & H &            & PII/CUI datastores \\
\code{data-backbone}      & data    & H & H & H & \checkmark & queues, brokers, the system-of-record DB \\
...
\item \textbf{Scope-amplifier escalation.} Certain archetypes (shared infrastructure)
  are flagged amplifiers and escalate to multi-agency scope when the offering serves
  more than one agency, regardless of the per-asset tag (Eq.~\ref{eq:scope}). This is
  our construct, not a FedRAMP one.
